...
int main(int argc, char* argv[]){
    int listenSocketFD, portNumber, establishedConnectionFD;
    ...
    while(1){
        ...
        spawnpid = fork();
            ...
            int holder = 0;
						...
						read(establishedConnectionFD, buffer, sizeof(buffer) - 1); //get info from client to see if we can decrypt
            ...
            char m, k; //used to get each char in plaintext and key
						...
						for (i = 0; i < l; i++) {
							...
							sum = valueText - valueKey; //subtract message and key to sum
              ...
              textMessage[i] =  alphabet[sum]; // convert to char
						...
						int messagelen = strlen(textMessage);
				...
				charsWritten = send(establishedConnectionFD, "@", strlen("@"), 0); //send terminator symbol
