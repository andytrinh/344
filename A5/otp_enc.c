...
int main(int argc, char* argv[]){
    ...
    int plainTextFile = atoi(argv[1]);
    FILE *fp;
    fp = fopen(argv[1], "r");
    while((c2 = fgetc(fp)) != EOF) {
      plainTextCount++;
      }
      plainTextLength = plainTextCount;
    ...
    while(read(charCounter, buffer, 1) != 0){
        for (i = 0; i < 27; i++) {
          if (buffer[0] == alphabet[i] || buffer[0] == ' ') {
            x = 1;
          }
        }
    ...
    //open plaintext and send contents to server
    FILE* fileText = fopen(argv[1], "r");
    int charsWritten1 = 0;
    char *plainTextMessage = malloc(sizeof(char) * 80000); //allocate memory for plaintext
    memset(plainTextMessage, '\0', 80000); //reinitialize
    size_t bufsize = 0; //size of buffer
    plainTextLength = getline(&plainTextMessage, &bufsize, fileText); //store content of plaintext into plainTextMessage
    plainTextMessage[strcspn(plainTextMessage, "\n")] = '@'; //replace newline with terminator symbol
    ...
    keyLength = getline(&keyMessage, &bufsize, fileKey); //store content of key into keyMessage
    ...
    char completeMessage[80000]; //message to be sent to server
